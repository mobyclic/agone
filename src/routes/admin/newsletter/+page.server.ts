...
import { listSubscribers, subscriberStats, seedFromUsers, unsubscribeByEmail, listNewsletterIssues } from '$lib/server/newsletter';
...
export const load: PageServerLoad = async ({ url }) => {
  ...
  const [{ subscribers, total }, stats, issues] = await Promise.all([
    listSubscribers({ q, status, limit: LIMIT, offset: (page - 1) * LIMIT }),
    subscriberStats(),
    listNewsletterIssues()
  ]);
  return { subscribers, total, stats, issues, q, status, page, limit: LIMIT };
};
