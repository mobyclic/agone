<script lang="ts">
  ...
  type Row = { id: string; name: string; slug: string; sort: number; count: number; published_count: number; visible: boolean };
  // Visibles (au moins un article publié) → réordonnables. Masquées → grisées en fin de liste.
  let rows = $state<Row[]>(untrack(() => data.rubriques.filter((r) => r.visible)));
  const hidden = $derived(data.rubriques.filter((r) => !r.visible));
...
<svelte:head><title>Catégories · Admin Agone</title></svelte:head>

<div class="mb-5 flex flex-wrap items-center justify-between gap-3">
  <div>
    <h2 class="text-xl font-bold">Catégories</h2>
    <p class="text-sm text-muted-foreground">Rubriques de l'Antichambre. Glissez pour réordonner, puis enregistrez l'ordre.</p>
  </div>
  <Button href="/admin/categories/nouvelle"><Plus size={16} /> Nouvelle catégorie</Button>
</div>
    ...
    {#each rows as c, i (c.id)}
      ...
      >
        ...
        <div class="min-w-0 flex-1">
          <a href="/admin/categories/{c.id}" class="font-medium hover:text-link">{c.name}</a>
          <span class="ml-2 font-mono text-xs text-muted-foreground">/{c.slug}</span>
        </div>
        <span class="shrink-0 text-sm text-muted-foreground" title="Publiés / total">{c.published_count}/{c.count} article{c.count > 1 ? 's' : ''}</span>
        <div class="flex shrink-0 items-center gap-1">
          ...
          <a href="/admin/categories/{c.id}" class="p-1 text-muted-foreground hover:text-foreground" aria-label="Éditer"><PencilSimple size={15} /></a>
        </div>
      </li>
    {/each}
    {#if rows.length === 0}
      <li class="px-3 py-10 text-center text-muted-foreground">Aucune catégorie affichée.</li>
    {/if}
  </ul>
</div>
...
{#if hidden.length}
  <div class="mt-6">
    <p class="mb-2 flex items-center gap-1.5 text-sm font-medium text-muted-foreground">
      <EyeSlash size={15} /> Non affichées sur le site <span class="font-normal opacity-70">— aucun article publié</span>
    </p>
      ...
      <ul class="divide-y divide-border">
        {#each hidden as c (c.id)}
          <li class="flex items-center gap-3 px-3 py-2.5 text-muted-foreground">
            <div class="min-w-0 flex-1">
              <a href="/admin/categories/{c.id}" class="font-medium hover:text-foreground">{c.name}</a>
              <span class="ml-2 font-mono text-xs text-muted-foreground/70">/{c.slug}</span>
            </div>
            <span class="shrink-0 text-sm">{c.published_count}/{c.count} article{c.count > 1 ? 's' : ''}</span>
            <a href="/admin/categories/{c.id}" class="p-1 hover:text-foreground" aria-label="Éditer"><PencilSimple size={15} /></a>
          </li>
        {/each}
      </ul>
    </div>
