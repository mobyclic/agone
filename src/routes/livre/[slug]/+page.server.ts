...
export const load: PageServerLoad = async ({ params, cookies, locals }) => {
  ...
  const [sameAuthor, contributions, sameCollection, cartSlugs, purchasedSlugs] = await Promise.all([
  ...
  return { book, sameAuthor, contributions, sameCollection, cartSlugs, purchasedSlugs, primaryAuthor: book.authors?.[0] };
};
